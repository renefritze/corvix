...
export function useKeyboard({ onRefresh, onFocusFilters, onDismissFocused }: KeyboardOptions) {
  useEffect(() => {
    function handleKeyDown(e: KeyboardEvent) {
      ...
      if (e.key === "j" || e.key === "J") {
        const rows = Array.from(document.querySelectorAll<HTMLElement>("tr[tabindex='0']"));
