...
| `GET` | `/api/health` | Always public. Returns `200` with `{"status": "ok"}`; returns `503` with `{"status": "unhealthy"}` when unhealthy (no further detail — see `/api/v1/snapshot` for poller error detail). |
